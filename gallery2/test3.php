...
foreach ($children as $child) {
	list($error, $thumbnails) = GalleryCoreApi::fetchThumbnailsByItemIds(array($child->getId()));
	if ($error) {
		print("Could not fetch thumbnails\n");
		print_r($error);
		exit("</pre>");
	}
	foreach ($thumbnails as $thumbnail) {
?>
		<img src="/gallery2/d/<?php print($thumbnail->getId()); ?>-<?php print($thumbnail->getSerialNumber()); ?>/<?php print($child->getPathComponent()); ?>" width="<?php print($thumbnail->getWidth()); ?>" height="<?php print($thumbnail->getHeight()); ?>" alt="<?php print($child->getTitle()); ?>" />
<?php
	}
}
?>
